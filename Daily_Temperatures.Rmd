...
TLH.df |>
  group_by(doy) |>
  summarize( AvgByDoY = mean(DTR),
             SDByDoY = sd(DTR),
             SkewByDoY = skewness(DTR) ) |>
...
#  filter(Year >= 1990) |>
  group_by(doy) |>
  summarize(pvalueS = shapiro.test(DTR)$p.value,
            pvalueAD = ad.test(DTR)$p.value) 
