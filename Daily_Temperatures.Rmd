...
ALB.df <- read_csv(file = 'Data/ALB_Daily1891-2020.csv') |>
  mutate(Date = as.Date(DATE)) |>
  mutate(Year = year(Date), 
         month = month(Date, label = TRUE, abbr = TRUE),
         doy = yday(Date),
...
# TLH.df <- ALB.df  # Albany, GA shows more cooling days than warming days. Why? Try another station?
