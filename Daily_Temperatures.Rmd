...
Changes <- TLH.df |> 
  ...
  mutate(Difference = Temperature[Year == 2020] - Temperature[Year == 1950],
         Trend = case_when(Difference < 0 ~ "blue",
                           TRUE ~ "red")) |>
  as.data.frame()
```
...
breaks_y <- 366 * seq(.05, .25, by = .05)
labels_y <- c(paste0(seq(.05, .25, by = .05) * 100, "%"))

Trends |>
  mutate(Change = estimate * (2020 - 1943 + 1),
         bins = cut(Change, seq(-12, 12, by = 1))) |>
  group_by(bins) |>
  ggplot(mapping = aes(x = Change)) +
    ...
    scale_color_gradient2(low = muted("blue"),
                          ...
                          midpoint = 0,
                          guide = 'none') +
    labs(title = "More days of the year have warmed than cooled in Tallahassee, FL",
...
Next fit a linear model to the first doy.
```{r}

fit0.1 <- lm(formula = MaxTemp ~ Year, 
           data = by_doy$data[[1]])
fit0.2 <- update(fit0.1,
