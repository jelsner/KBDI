...
joineddata.df %>%
  ...
  pull(cc)
```
Plotting Ql for TLH and APL 1999
...
1999 is the year with the highest cc, plot Ql for TLH vs APL and see how months vary
...
ggplot(aes(x = Date)) +
  ...
  facet_wrap(~ Year.x, scales = "free_x")
```
Further Filtering summer dry period: June - August

```{r}
joineddata.df %>%
  filter(month.x == 'Jun' | month.x == 'Jul' | month.x == 'Aug') %>%
ggplot(aes(x = Date)) +
  geom_line(aes(y = Ql.x), color = "darkred") +
  geom_line(aes(y = Ql.y), color = "steelblue") +
  facet_wrap(~ Year.x, scales = "free_x")
...
Comparing Correlation Coefficient grouped by Month
...
joineddata.df %>%
  group_by(month.x) %>%
  summarise(cc = cor(Ql.x, Ql.y)) %>%
ggplot(mapping = aes(x = month.x, y = cc)) +
  geom_point()
```

Comparing Correlation Coefficient grouped by year

```{r}
joineddata.df %>%
  group_by(Year.x) %>%
  summarise(cc = cor(Ql.x, Ql.y)) %>%
ggplot(mapping = aes(x = Year.x, y = cc)) +
  geom_point()
```
Filter out April which is the month of highest cc. Group by year
```{r}
...
ggplot(mapping = aes(x = Year.x, y = cc)) +
  geom_col()
  #geom_hline(yintercept = mean(cc))
```

```{r}
ggplot(data = joineddata.df, aes(x = Date)) +
  geom_line(aes(y = Ql.x), color = "darkred") +
  geom_line(aes(y = Ql.y), color = "steelblue")
```
```{r}
