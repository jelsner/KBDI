...
ggplot(mapping = aes(x = Date, y = Qlm, color = NAME)) +
  geom_line() 
```
Plot net rainfall for August 2003 - Febuary 2004
```{r}
CompareLong.df %>%
  filter(Date >= '2003-08-01') %>%
ggplot(mapping = aes(x = Date, y = NetR, color = NAME)) +
...
plot max temp for Augst 2003 - Febuary 2004
```{r}
CompareLong.df %>%
  filter(Date >= '2003-08-01') %>%
ggplot(mapping = aes(x = Date, y = MaxTemp, color = NAME)) +
...
TKAAFplot + 
  geom_line(mapping = aes(x = Date, y = Qlm.x)) +
  geom_bar(mapping = aes(x = Date, y = NetR.x), stat = "identity")
```
