...
Filter KAAF.df to the longest period of no missing data
Oct. 9 2002 - Jan. 30 2005

```{r}
KAAFcompare.df <- KAAF.df %>%
  filter(KAAF.df$Date >= "2002-10-09" & KAAF.df$Date <= "2005-01-30")
```

Filter TLH.df to match time perid of KAAFcompare.df

```{r}
TLHcompare.df <- TLH.df %>%
  filter(TLH.df$Date >= "2002-10-09" & TLH.df$Date <= "2005-01-30")
```
...
*Note, 3 missing precip days
Is this acceptable? Dates defined as 
```{r}
...
## Compute daily values of the KBDI For TLH
...
TLHcompare.df <- TLH.df %>%
 dplyr::filter(Date >= "2002-10-09" & Date <= "2005-01-30") #Create common Period with KAAF
range(TLHcompare.df$Ql)
```

### Compute daily values of the KBDI For KAAF
