...
```
## Filter the two the two data frames to have a common period of record
...
## Compute daily values of the KBDI For TLH From 2000 - 2012
...
TLHcompare.df <- TLH.df %>%
 dplyr::filter(Year >= 2000 & Year <= 2012) # Only comparing data from 2000 - 2012
range(TLHcompare.df$Ql)
```

### Compute daily values of the KBDI For KAAF From 2000 - 2012
...
KAAFcompare.df <- KAAF.df %>%
 dplyr::filter(Year >= 2000 & Year <= 2012) # Only comparing data from 2000 - 2013
...
joineddata.df %>% select(DATE, NAME.x, DroughtIndex.x, NAME.y, DroughtIndex.y)
```
