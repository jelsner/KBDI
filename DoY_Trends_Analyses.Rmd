...
Changes <- TLH.df |> 
  ...
  nest(data = -doy) |>
    mutate(fit = map(data, ~lm(MinTemp ~ Year, data = .x)),
           augmented = map(fit, augment)) |>
    unnest(augmented) |>
  filter(Year %in% c(1948, 2020)) |> # start and end must be leap years
  mutate(Year = factor(Year, levels = c(1948, 2020), ordered = TRUE),
  ...
  group_by(DayMonth) |>
  mutate(Difference = Temperature[Year == 2020] - Temperature[Year == 1948],
