...
exp(coef(glm.nb(nF ~ KBDI, data = df))) # 1.189 or 19% per cm increase in soil moisture deficit
...
Lightning.sf <- st_read(dsn = "NorthFloridaLightningHexagonsPos.shp") %>%
  st_transform(crs = st_crs(LandHex.s)) %>%
  mutate(Year = year(Date),
         Month = month(Date),
         Day = day(Date))
head(Lightning.sf)
...
Not very enlightening as avgTmax responds to precipitation rather than precipitation as a response to temperature.
