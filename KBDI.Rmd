...
Number of days per year with KBDI above 400.
```{r}
TLH.df %>%
  filter(Year != 2020) %>%
  group_by(Year) %>%
  summarize(nD = sum(Ql > 400)) %>%
  ggplot(aes(Year, nD)) +
  geom_point() +
...
Fires.sf <- st_intersection(LandHex.sf, Fires.sf)
...
Fires.sf <- Fires.sf %>%
  ...
  select(Year, Month, Day, YDay, YDayF)
plot(Fires.sf$geometry)
```
...
ggplot(Monthly, aes(x = MonthF, y = nF)) +
