...
Some preliminary work here but copy/paste to a GitHub repo.

Original paper: https://www.srs.fs.usda.gov/pubs/rp/rp_se038.pdf

Also see hot-days.Rmd in hot-days repo on GitHub.
...
( DeltaQ <- (800 - Q) * (.968 * exp(.0486 * MaxT) - .830) /(1 + 10.88 * exp(-.0441 * R)) * .001 )
```
...
DeltaQl <- numeric()
for(i in 1:30){
  DeltaQ <- (800 - Q) * (.968 * exp(.0486 * MaxTemp[i]) - .83) /(1 + 10.88 * exp(-.0441 * R)) * .001 
...
### Repeat for a month of data from Tallahassee airport.
