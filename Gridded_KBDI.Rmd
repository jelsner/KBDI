...
Start with one year. 2017.

## PRISM data

Download daily PRISM data using functions from the {prism} package. About 35 (23) minutes per year per variable of daily data at home (apartment) internet speeds. Temperatures are in C and precipitation is in mm.
```{r}
...
X <- filter(tmax, time == ymd("2017-01-01"))
plot(X)
```
...
ANF_BBox.sfc <- 
  ...
  st_as_sfc()
=======
TP.st <- TP.st[ANF_Boundary.sf]
TP3.st <- TP.st |>
  st_crop(ANF_Boundary.sf)
>>>>>>> 57fd2cf5dafb4504949b99ec0ce9eff062e201d7

X <- filter(TP3.st["Tmax_C"], time == ymd("2017-01-01"))
X <- round(X, digits = 0)
plot(X, text_values = TRUE)
plot(ANF_Boundary.sf, add = TRUE, col = "transparent")
```
...
Check with plot
```{r}
X <- filter(TP.st, time >= ymd("2017-08-01"), time <= ymd("2017-08-31"))

ggplot() +  
  geom_stars(data = X["Tmax_C"], alpha = .8, downsample = c(1, 1, 1)) + 
  facet_wrap("time") +
...
<<<<<<< HEAD
Convert temperatures in C to F and precipitation in mm to inches for use in KBDI calculations.
=======
Compute KBDI for each cell. Start by converting temperatures in C to F and precipitation in mm to inches
>>>>>>> 57fd2cf5dafb4504949b99ec0ce9eff062e201d7
```{r}
TP.st <- 
  TP.st |>
...
Check with a plot
```{r}
X <- filter(TP.st, time >= ymd("2017-10-01"), time <= ymd("2017-10-31"))

ggplot() +  
  geom_stars(data = X["Qlm"], alpha = .8, downsample = c(1, 1, 1)) + 
  facet_wrap("time") +
...
ggplot() +  
  geom_stars(data = X["DroughtIndex"], alpha = .8, downsample = c(1, 1, 1)) + 
  facet_wrap("time") +
#  scale_fill_distiller(palette = "RdBu") +
...
ggplot() +  
  geom_stars(data = R.st["A1"], alpha = .8, downsample = c(1, 1, 1)) + 
...
Use the {superpixel} package to look for patterns in the correlation.
