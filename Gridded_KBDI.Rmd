...
Create a {stars} object using high temperature (`tmax`) and daily rainfall (`ppt`). About 4 minutes to read ten years of daily temperature and rainfall.
```{r}
ts <- proc.time()
t0 <- 2009
t1 <- 2018
...
X <- filter(TP.st["Tmax_C"], time == ymd("2010-01-10"))
X <- round(X, digits = 0)
...
Compute daily drought index at each grid point. About 20 seconds.
```{r}
...
Check with maps.
```{r}
X <- filter(TP.st, time >= ymd("2010-04-01"), time <= ymd("2010-04-08"))
...
TLH.df <- read.csv(file = 'Data/TLH_Daily1940-2020.csv',
  ...
  filter(Year >= t0 & Year <= t1) # to match the PRISM years
...
ggplot() +  
  geom_stars(data = R.st["R^2"], alpha = .8) + 
...
ggplot() +
  geom_sf(data = FL_Counties.sf, fill = "transparent", 
          col = "gray80") +
  geom_stars(data = R.st) +
...
ggplot() +
  ...
  geom_sf(data = R.sf, 
          color = 'transparent',
          mapping = aes(fill = as.character(`R^2`))) +
