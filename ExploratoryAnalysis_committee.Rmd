...
Attempt to read file in directly from wesite. Still recieving errors.
```{r}
...
Fire set downloaded and pulled from file rather than directly from the website. Had errors when trying to import it directly from the website. Downloaded and converted to more user friendly shapefiles in ESRI.
...
Select column names need to be adjusted
```{r}
...
```{r}
AprilKBDI <- rangeQ1991.df %>%
  filter(year(Date) >= 1992) %>%
  filter(month(Date) == 04) %>%
  mutate(Season = year(Date)) %>%
  group_by(Season) %>%
  summarise(April_Avg = mean(Q400), April_Median = median(Q400), April_Min = min(Q400), April_Max = max(Q400), KBDI_LastDay = Q400[month(Date) == 04 & day(Date) == 30])
```
