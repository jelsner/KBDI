...
```{r}
countfiresandburned.df <- left_join(x = burnedAcers, y = seasonFires, by = c("forecastYear" = "Year"))
...
cor(countfiresandburned.df$acersBurned, countfiresandburned.df$nFIRES)
```
...
```{r}
testgroup4 <- allFires.sf %>%
  group_by(FIRE_YEAR, month(DISCOVERY_)) %>%
  summarise(acersBurned = sum(FIRE_SIZE[month == "1" | month == "2" | month == "3"])) + mutate((FIRE_YEAR = FIRE_YEAR -1(sum(FIRE_SIZE[month == "5"]))))
```

follow example:
PrecipAnom = sum(Rainfall24[month == "Jan"| month == "Feb"| month == "Mar"| month == "Apr"])-17.46,
```{r}
testgroup1 <- testgroup %>%
  group_by(FIRE_YEAR, FIRE_YEAR+1) %>%
  summarise(acersBurned = sum(monthAcers[month == "1" | month == "2" | month == "3"])) + sum(monthAcers[FIRE_YEAR+1] & monthAcers[month == "4"])
```
...
Now find a way to group fires by the new periods and sum the total burned area for that period.
```{r}
testgroup1 <- allFires.sf %>%
  group_by(burnedGroups)
```
