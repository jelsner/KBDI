...
April Fire Distribution - Only 6 fires across all April months in the data set?
```{r}
par(mfrow=c(1,2))

AprilFires.ppp %>%
  plot(main = "April Fires")
AprilFires.ppp %>%
  density() %>%
  plot(main = "Spatial Distribution - April Fires")
```
...
There is a low correlation (0.399) between the average KBDI in April and the Average KBDI of the proceeding summer season
```{r}
TLH.df%>%
...
April Precip Anomaly is negatively correlated to April Average KBDI (-.62). The further below the average rainfall total, the higher likelihood of a high April KBDI average.
April Temp Anomaly is positively correlated to April Average KBDI (+0.37). This is a low correlation coeffiecnt and departure from the average April temperature does not appear as significant as the departure from the average April rainfall.
```{r}
TLH.df %>%
  filter(month == "Apr") %>%
  filter(Year >= 1991 & Year <= 2020) %>%
  group_by(Year) %>%
  summarise(TempAvg = mean(TMAX), Precip = sum(Rainfall24), TempAnom = mean(TMAX)-80.2, PrecipAnom = sum(Rainfall24)-3.53, AvgKBDI = mean(Ql)) %>%
  summarise(PrecipAnomKBDI = cor(PrecipAnom, AvgKBDI), TempAnomKBDI = cor(TempAnom, AvgKBDI))
```
...
Exploring a longer period of rainfall deficit, a correlation coefficient of -0.56 is found. This is slightly lower than comparing the rainfall deficit across just April which was -0.62
```{r}
TLH.df %>%
...
AprilKBDI <- TLH.df %>%
  ...
  mutate(Season = year(Date)) %>%
  group_by(Season) %>%
  summarise(April_Avg = mean(Ql), April_Median = median(Ql), April_Min = min(Ql), April_Max = max(Ql), KBDI_LastDay = Ql[month(Date) == 04 & day(Date) == 30])
```

Merge fireseason fire counts with the April KBDI summaries 
```{r}
AprilStats <- left_join(x = seasonFires, y = AprilKBDI, by = c("Year" = "Season")) %>%
  rename(Season = Year, SeasonFires = nFIRES)
```

When exploring the relationship between April KBDI values and the number of fires that occured in the proceding season a correlation coefficient of about 0.55 is found.
```{r}
...
Can't use binomial family is using AprilStats data set. Use poison because SeasonFires is count data.
...
A summary of April GLM shows that the Average April KBDI is statisically significant.
```{r}
...
```{r}
AprilStats$PredFires <- pred_AprilGLM$fit
AprilStats$Compare <- AprilStats$SeasonFires - AprilStats$PredFires
cor(AprilStats$SeasonFires, AprilStats$PredFires)
```
...
We find that Average KBDI is not statistically significant
correlation is slightly stronger than mode that was run without max and min KBDI
```{r}
...
```{r}
TLH.df %>%
  filter(month == "May"| month == "June"| month == "July") %>%
  group_by(Year) %>%
  ...
  labs(title = "Annual Rainfall During the Fire Season is Decreasing",
...
TLH.df %>%
  ...
  labs(title = "The Average Monthly Temperature at the Tallahassee International Airport is Rising",
