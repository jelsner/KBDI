...
Import Forest Boundaries to explore lightning data across ANF
...
Get lightning data for state of Florida
...
lightningdata <- data.frame()
for(i in 1:length(fn)){
  X <- read.csv(fn[i], na.strings = "NULL", header = TRUE, stringsAsFactors = FALSE)
  lightningdata <- rbind(lightningdata, X)
}
