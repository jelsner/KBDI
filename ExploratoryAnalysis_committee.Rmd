...
```{r}
if(!"updatedFires" %in% list.files()){
download.file("https://www.fs.usda.gov/rds/archive/products/RDS-2013-0009.5/RDS-2013-0009.5_GDB.zip",
"updatedFires.zip")
unzip("updatedFires.zip")
}
updatedFires.sf <- st_read(dsn = "updatedFires.zip") %>%
st_transform(crs = 3086)

#filtered fires within ANF bounds
anf_fires.sf <- st_join(updatedFL_Fires.sf, anfbounds.sf, join = st_within) %>%
  filter(NFSLANDU_2 == 'Apalachicola National Forest')
...
create data set containing fires and lightning counts. Match years for data sets (1992 - 2013)
...
Note: There are no days with a Qlm value above 200 in this set of data (1992-2012)
```{r}
...
ggplot(mapping = aes(x = merge2.df$Qlm, y = pred_glmQlm$fit)) +
  geom_ribbon(aes(ymin = lowerQlm, ymax = upperQlm), fill = "grey") +
  geom_line(color = "blue") +
  ylab("Predicited Probablility") +
  xlab("Daily Qlm") +
  labs(
