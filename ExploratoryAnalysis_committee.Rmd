...
lightningdata1.df <- lightningdata1.df %>%
  mutate(SEQDAY = as.Date(SEQDAY),
         ...
         MONTH = month(SEQDAY),
         YEAR = year(SEQDAY)) %>%
#missing data beyond 5/20/2013. Remove last 5 rows from data set. This addresses the parsing failure as well
...
Note: This plot includes days with zero lightning strikes and concludes that on days with zero lightning strikes there is a 2% chance of a lightning ignited wildfire occurring. A lightning fire cannot occur without lighting. Should be zero percent.
```{r}
library(ggplot2)
...
Filter data for only fire season before running glm.

When filtering to only the fire season, there is more uncertianty, and the model is closer to linear.
```{r}
...
General linear regression - Study how Qlm impacts lightning wildfire occurrence.
```{r}
glmQlm <- glm(formula = FireBool ~ Qlm, family = binomial, data = merge2.df)

pred_glmQlm <- predict(object = glmQlm,
                      type = "response",
                      se.fit = TRUE)

lowerQlm <- pred_glmQlm$fit - (1.96*pred_glmQlm$se.fit)
upperQlm <- pred_glmQlm$fit + (1.96*pred_glmQlm$se.fit)

ggplot(mapping = aes(x = merge2.df$Qlm, y = pred_glmQlm$fit)) +
  #geom_ribbon(aes(ymin = lower, ymax = upper), fill = "grey") +
  geom_line(color = "blue") +
  ylab("Predicited Probablility") +
  xlab("Daily Qlm") +
  labs(
    title = "Predicted Probability of Fire Occurance Based on Daily Qlm Values",
    subtitle = "With 95% Confidence Interval",
...
```{r}
GLM <- glm(formula = FireBool ~ LightningCount + Qlm, family = binomial, data = merge2.df)
