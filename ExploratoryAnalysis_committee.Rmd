...
###Compare predicted number of fires for a season with actual values

Explore KBDI by season

Merge KBDI and Fire data sets. The rangeQ1991.df data set is used to represent KBDI. Because all KBDI variables merge prior to 1992, it does not matter which KBDI column is used. Q400 will be used here.

Create stat columns for KBDI for each season
```{r}
seasonKBDI <- rangeQ1991.df %>%
  filter(year(Date) >= 1992) %>%
  filter(month(Date) == 05 | month(Date) == 06 | month(Date) == 07) %>%
  mutate(Season = year(Date)) %>%
  group_by(Season) %>%
  summarise(Season_Avg = mean(Q400), Season_Median = median(Q400), Season_Min = min(Q400), Season_Max = max(Q400))
```
Reworked using TLH.df. Compare to make sure they are equivalent
```{r}
seasonKBDI <- TLH.df %>%
  filter(year(Date) >= 1992) %>%
  filter(year(Date) <= 2018) %>%
...
Check correlations across data
```{r}
...
Check correlations across April data
```{r}
...
correlation is slightly stronger than without max and min KBDI
```{r}
...
create dataframe with both fire and KBDI data
```{r}
...
Explore the percentage of wildfires that occur in each KBDI catagory 
```{r}
...
In the years that were noted to be abnormally hot and dry (2002 & 2011) KBDI values for the summer months were high. Infact, monthly mean KBDI values were higher than they were in April in both cases
```{r}
TLH.df%>%
  #filter(month == "May"| month == "Jun"|month == "Jul") %>%
  filter(Year >= 1991 & Year <= 2020) %>%
  group_by(Year) %>% #, month) %>%
  summarise(AvgKBDI = mean(Ql))
```
...
The relationship between summer KBDI averages is not as clear when comparing it to rainfall deficits from Jan - Apr
```{r}
TLH.df %>%
  filter(month == "Jan"| month == "Feb" | month == "Mar"| month == "Apr") %>%
  filter(Year >= 1991 & Year <= 2020) %>%
  group_by(Year) %>%
  summarise(PrecipAnom = sum(Rainfall24)-17.46)
```
