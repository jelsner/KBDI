...
Future work: Make fire size an ordered factor and create a model that predicts the probability by size category similar to what we did for predicting tornado EF rating.
```{r}
NaturalFires.sf |>
  group_by(SizeF) |>
  summarize(nFires = n(),
...
p1 <- ggplot(data = df,
        ...
        theme_minimal() +
        labs(x = "", y = "(cm)",
             title = "Monthly rainfall and average daily high temperature",
             subtitle = "NWSFO TLH: 1943-2020")
...
ggplot(data = df,
  ...
  geom_point() +
  geom_label_repel() +
  labs(title = "The number of seasonal fires increases with increasing April soil dryness",
       x = "KBDI on April 30th", y = "Number of fires in the ANF (May-Jul)") +
  theme_minimal()
```
...
TLH.df |>
  ...
  group_by(doy, Year) |>
  mutate(Qlm = Qlm) |>
  dplyr::select(Year, doy, Qlm, Date) |>
