---
title: "Lightning-Kindled Wildfires in the Apalachicola National Forest (ANF)"
output: html_document
editor_options: 
  chunk_output_type: console
---

Prescribed fire NOT prescribed burn. Lots of resources are required for planning and implementing a prescribed fire. Seasonal forecasts can help with planning of those resources.

fires remove fuels for subsequent fires, 

What is the relationship between dryness of the soil and lightning-kindled wildfire activity in the ANF on the seasonal time scale? Quantifying the statistical relationship between soil dryness and lightning-kindled wildfire activity in the ANF.

(1) By how much does the threat of a wildfire increase during the fire season with increases in soil dryness during the dry season?
(2) What climate factors are associated with soil dryness during the dry season?
(3) Is there an increasing risk of wildfires in the ANF?
...
KBDI assesses the risk of fire. KBDI is computed from daily rainfall and high temperature. These data are available at TLH NWS. Establish that TLH represents well the ANF by showing a correlation map between KBDI computed at TLH and PRISM data across the ANF. The PRISM dataset should not be used to calculate century-long climate trends due to non-climatic variations from to station equipment and location changes, openings and closings, varying observation times, and the use of relatively short-term networks.
...
NOT USED: See also NASA burned-area data https://firms.modaps.eosdis.nasa.gov/
```{r, eval=FALSE}
...
Possible project: Make fire size an ordered factor and create a model that predicts the probability by size category similar to what we did for predicting tornado EF rating.
```{r}
NaturalFires.sf %>%
  group_by(SizeF) %>%
  summarize(nFires = n(),
...
Combine seasonal fire data with dryness data.
```{r}
...
```{r}
cor(df$AvgQlm, df$totalAcresBurned) # .54
cor(df$QlmLastDay, df$totalAcresBurned) #.48
cor(df$QlmLastDay, df$nFires) # .60
