...
How much skill is there in a prediction of the seasonal frequency and intensity of lightning-sparked wildfires in the ANF using the KDBI?


Title: Predicting the seasonal frequency and severity of lightning-sparked wildfires in the Apalachicola National Forest
...
Area is 2,564 square km. There are 337 lightning-sparked wildfires for an average intensity of 13.1 fires per 10 square kilometer during May-July over the 24-year period.
```{r}
LF.ppp %>%
  density() %>%
  plot()
...
ggplot(data = df,
  ...
  labs(title = "Locations of lightning-sparked wildfires in the ANF are inconsistent with a random process",
...
Seasonal number of lightning-sparked fires and seasonal total acres burned.
```{r}
df <- LightningFires.sf %>%
  st_drop_geometry() %>%
  filter(Month %in% c(5, 6, 7)) %>%
...
ggplot(data = df,
  ...
  theme_minimal() +
  labs(x = "", y = "",
       title = "Large inter-seasonal variability in the number of lightning-sparked wildfires in the ANF",
       subtitle = "Darker color points indicates larger total burn area")
...
ggplot(data = df,
  ...
  theme_minimal() +
  labs(x = "", y = "",
       title = "Large inter-seasonal variability in acres burned from lightning-sparked wildfires in the ANF",
       subtitle = "Darker color points indicates more fires")
```
...
#  scale_y_reverse(breaks = seq(0, 200, by = 50), labels = c("Wet", 50, 100, 150, "Dry")) +
  scale_color_gradientn(colors = terrain.colors(5), guide = FALSE) +
  ylab("") + xlab("") +
