...
XX <- left_join(X, Yearly, by = "Year")

XX %>%
  group_by(Day) %>%
  summarize(r = cor.test(KBDI, nF)$estimate,
            rLo = cor.test(KBDI, nF, conf.level = .9)$conf.int[1],
            rHi = cor.test(KBDI, nF, conf.level = .9)$conf.int[2]) %>%
