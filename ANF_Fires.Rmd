...
sum(is.na(TLH.df$PRCP)) / nrow(TLH.df) * 100  # only 5 missing values [~ .0171% of all days]
...
( AnnualRainfall <- TLH.df %>%
  filter(Year > 1940) %>%  # data started in March 1940
  group_by(Year) %>%
...
TLH.df <- TLH.df %>%
 dplyr::filter(Year >= 1946 & Year <= 2019) # Only full years after the daily Q values have converged
range(TLH.df$Ql)
```
...
#  scale_fill_gradient2(low = "#FC8B93", mid = "#965784", high = "#D9DE6E", guide = 'none') +
  labs(x = "", y = "",
       ...
       caption = "Period of record: 1946-2019, Data source: NWSFO Tallahassee") +
  theme_dark() 
```
...
TLH.df %>%
  ...
  theme_dark() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Dry months are getting drier in the Apalachicola National Forest.",
...
TLH.df %>%
  ...
  scale_color_manual(labels = c("1946-1980", "1981-2019"), values = c("black", "sandybrown")) +
