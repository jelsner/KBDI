...
lightningAvg.df %>%
  ...
  labs(x = "", y = "",
       ...
       subtitle = "Average Number of Lightning Strikes Per Month",
...
```{r}
formula1 <- lm(nFIRES ~ Ql_April30, data = seasonFires.sf)
#glm1 <- glm(formula1, data = seasonFires.sf, family = poisson)
```

create a generalized linear model (Poisson)
```{r}
glm <- glm(nFIRES ~ Ql_April30, data = seasonFires.sf)
```
...
```{r}
ggplot(glm1, aes(x=Ql_April30, y = nFIRES)) +
  geom_point()+
  geom_smooth(method = "glm") +
  ylab("Number of Fires") +
...
```{r}
seasonFires.sf$glm1_Pred <- glm1$fitted.values
```

view fire plot with predicted values based on april KBDI (fitted values of the model)
```{r}
ggplot(seasonFires.sf, aes(x = Ql_April30, y = glm1_Pred)) +
  geom_point() +
  geom_smooth(method = "glm")+
  stat_regline_equation() +
  ylab("Predicted Number of Fires") +
...
```{r}
ggplot(seasonFires.sf, aes(x = Ql_April30, y = nFIRES)) +
  geom_point() +
  geom_smooth(method = "glm", formula = y ~ x,
              method.args = list(family = poisson(link = "log"))) +
...
temp <- seasonFires.sf %>%
  filter(Year >= 1993)
formula2 <- lm(nFIRES ~ Ql_April30 + acresBurned, data = temp)
...
glm (poisson)
```{r}
glm2 <- glm(nFIRES ~ Ql_April30 + acresBurned, data = temp)
