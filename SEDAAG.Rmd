...
Include a Ql centimeters column in the TLH.df data frame
```{r}
TLH.df <- TLH.df %>%
  mutate(Qlcm = Qlm/10)
```
...
(p1 / p2) +
  labs(caption = "Period of record: 1943-2020, Data source: NWSFO Tallahassee")
```
adjustments to plot above
```{r}
df <- TLH.df %>%
...
p2 <- ggplot(data = lightningAvg.df,
       mapping = aes(x = MonthF, 
                     y = avgLightningCount,
                     fill = avgLightningCount)) +
  geom_col() +
...
this can be edited in the future because I have now included a cm column in the base data frame (TLH.df)
...
visualize data
```{r}
ggplot(mapping = aes(y= logistic_DL$fit, x= logisticModel.df$Qlcm, color= logisticModel.df$LightningCount))+
  geom_point()+
  ylab("Predicited Probablility\nof a Fire Occurrence") +
  xlab("Daily Soil Moisture Deficit (cm)") +
  labs(color = "Daily Lightning Strike Count") +
  theme_minimal()
```
...
Adjustments to binomial model with two predictors (Soil Dryness and Lightning count).
...
#creating confidence interval
low_boundSM <- pred_SM$fit - (1.96*pred$se.fit)
up_boundSM <- pred_SM$fit + (1.96*pred$se.fit)
...
ggplot(newdat, aes(x = Qlcm, y = LightningCount, fill = br)) + 
        ...
        xlab("Soil Moisture Deficit (cm)") + ylab("Lightning Strike Count") +
        theme_minimal() +
...
Clean up data frame to be exported to table in paper
```{r}
appendix_table.df <- temp_cm %>%
  dplyr::select("Year":"acresBurned", "nbr1_fit", "nbr2_fit") %>%
  rename(NumberOfFires = nFIRES, SoilMoistureDeficit_cm = Qlcm, BurnedGroupInteval = burnedGroupInterval, AcresBurned = acresBurned, Model1Fit = nbr1_fit, Model2Fit = nbr2_fit)
```

Export the data frame to an excel table
```{r}
write_csv(appendix_table.df, "C:\\Users\\zlaw9\\OneDrive\\Thesis\\Thesis\\Paper Figures\\appendix\\appendixtable.csv")
```
...
TLH_1943.df %>%
  ...
  ggplot(aes(Year, nD)) +
  geom_point() +
  labs(x = "", y = "Number of days\nSoil moisture deficit exceeded 16 cm") +
