...
seasonFires_cm.sf <- left_join(x = seasonFires_cm.sf, y = burnedArea.sf, by = c("Year" = "forecastYear")) %>%
  dplyr::select("seasonFires_cm.sf$Year":"seasonFires_cm.sf$percentBurned") %>%
  rename(nFIRES = nFIRES.x)
```
...
```{r}
temp_cm$change_fit <- temp$nbr1_fit - temp$nbr2_fit
```
...
Trying file types to export plots to word
```{r}
#library(devEMF)

emf(file = "C:/Users/zlaw9/OneDrive/Thesis/Thesis/Paper Figures/emf figures/pdf_trial")
