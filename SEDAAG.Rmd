...
KBDI_fire.df <- left_join(x = TLH_fireYrs.df, y = countfires.df, by = c("Date" = "Date")) %>%
  select(Date, FireCount, Ql, DroughtIndex)
...
```{r}
temp.df <- data.frame(KBDI_April30 = 350.1028) #mean KBDI value
...
Exploring the confidence intervals
```{r}
temp.df <- data.frame(KBDI_April30 = 559.35924, acresBurned = 2034.00)
